...
from dataset.data_generator import get_image
...
class RAWsRGBSamplingDataset(Dataset):
    ...
    def read_img(self, raw_image_path, srgb_image_path):
        ...
        file_type = raw_image_path.rpartition('.')[-1]
        MAX_BIT = 2 ** 16 - 1
        raw_demosaic = get_image(raw_image_path) / MAX_BIT 
        MAX_BIT = 2 ** 8 - 1 if file_type == "png" else 2 ** 16 - 1 
        srgb_img     = get_image(srgb_image_path) / MAX_BIT

        if file_type == "png":
            if srgb_img.shape[0] > srgb_img.shape[1]:
                raw_demosaic = np.rot90(raw_demosaic, k=1)
