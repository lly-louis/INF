...
class INF(nn.Module):
    def __init__(self, num_layers, hidden_dim, add_layer, weight_decay=None):
        ...
        self.params += [{'params':self.color_net.parameters(),'weight_decay':weight_decay[1]}]
        self.params += [{'params':self.output_net.parameters(),'weight_decay':weight_decay[2]}]

    def forward(self, rgb, spatial):
        return self.output_net(torch.cat((self.color_net(rgb), self.spatial_net(spatial)), -1))